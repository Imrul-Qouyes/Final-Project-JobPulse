              ...
              <div class="p-4 ">
                  <form class="space-y-4" action="#">
                      <div>
                          ...
                          <input type="email" name="email" id="candidateEmail"
...
<script>
  ...
  async function sendOTP(event){
    ...
    if(candidateEmail.length === 0){
      ...
      showLoader();
      let res = await axios.post('/candidate/sendOTP',{
        email:candidateEmail
      });
      hideLoader();

      if (res.status === 200 && res.data.status === 'success') {
        sessionStorage.setItem('email',candidateEmail);
        successToast(res.data.message);
        window.location.href='/candidate/OTPVerify';
      }else{
